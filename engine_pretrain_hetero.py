...
def train_one_epoch(model: torch.nn.Module,
    ...
    total_iterations = 2 * max(len(data_loader_A), len(data_loader_B))

    for data_iter_step in range(total_iterations):
        # Alternate between dataset A (even iterations) and dataset B (odd iterations).
        if data_iter_step % 2 == 0:
            try:
                samples, _ = next(iter_A)
            except StopIteration:
                # Restart iterator for loader A if exhausted.
                iter_A = iter(data_loader_A)
                samples, _ = next(iter_A)
        else:
            try:
                ...
                # Restart iterator for loader B if exhausted.
                iter_B = iter(data_loader_B)
                samples, _ = next(iter_B)
